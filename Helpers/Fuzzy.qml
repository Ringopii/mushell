...
Singleton {
    ...
    for (let i=1; i <= b.length; i++) {
      for (let j=1; j <= a.length; j++) {
        ...
        if (b.charAt(i-1) === a.charAt(j-1)) {
          ...
          matrix[i][j] = Math.min(
            matrix[i-1][j-1] + 1,  // Substitution: replace b[i-1] with a[j-1]
  ...
  function getScore(q: string, t: string): real {
    return (
      (distanceScore(q,t) * distanceWeight) +
      (prefixScore(q,t) * prefixWeight) +
      (consecutiveScore(q,t) * consecutiveWeight)
    )
  }
